...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('therapeutic_relationship_participants', (table) => {
    table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
  ...
  await knex.schema.createTable('recurrence_patterns', (table) => {
    ...
    table.foreign('recurrence_end_type').references('name').inTable('recurrence_end_types').onDelete('RESTRICT');
  ...
  await knex.schema.createTable('calendar_sessions', (table) => {
    ...
    table.foreign('session_status').references('name').inTable('session_statuses').onDelete('RESTRICT');
    table.string('billing_status');
    table.foreign('billing_status').references('name').inTable('billing_statuses').onDelete('RESTRICT');
